...
const inputText2 = document.querySelector(".about-hidden-results-container");
const hiddenCategories = document.querySelector(".about-hidden-categories");
...
function handleSearchResults(filteredItems, inputedItem, container, imgWidth) {
  ...
  function displayMore() {
    ...
    toDisplay.forEach((item) => {
      const newDiv = document.createElement("div");
      newDiv.innerHTML = `
      <div class="about-results-container">
      <div>
      <p>${item.name}</p>
      <p style="font-weight: bold;" >${item.price}</p>
      </div>
      ...
      `;
const resultsContainer = newDiv.querySelector(".about-results-container");
      resultsContainer.style.cursor = "pointer";
      resultsContainer.addEventListener("click", () => {
        localStorage.setItem("selectedProduct", JSON.stringify(item));
        window.location.href = `product.html`;
