...
// Reuse the babycare dataset but render it with the same UI/logic as shop.js
...
const AllProducts2 = [...provisions, ...cosmetics, ...toiletries, ...babycareProducts];

// Split dataset into UI filter groups based on image folder prefix
const babyWipesProducts = babycareProducts.filter(
  (p) => p.image && p.image.startsWith("Baby Wipes/"),
);
const babyFoodProducts = babycareProducts.filter(
  (p) => p.image && p.image.startsWith("Baby Food/"),
);

const AllProducts = [...babyWipesProducts, ...babyFoodProducts];
...
// Sidebar elements (shop.html structure)
...
function toggleMenu(show) {
  if (!menuBtn || !hiddenMenu || !hiddenClose) return;
  if (show) {
...
function showWishlistModal(image, name, status) {
  ...
  const imageIcon =
    status === "added" ? "check.png" : status === "removed" ? "multiply.png" : "multiply.png";
...
function eachProductContainer(theResultsItem) {
  ...
  if (heartIcon) {
    ...
    const existingIndex = wishlist.findIndex((item) => item.name === EachProductName);
    if (existingIndex !== -1) {
      heartIcon.classList.add("fa-solid", "added-to-wishlist");
      heartIcon.style.color = "red";
    }
  ...
  // store for optional future use
  productItemDiv.dataset.category = EachProductCategory;
...
function displayProducts(productsToDisplay) {
  ...
  productsToDisplay.forEach((product) => eachProductContainer(product));

  const cumulativeShowing = Math.min(currentPage * itemsPerPage, currentProducts.length);
  if (displayResult) {
...
function filterItems(item) {
  // Update the UI header text based on current filter
  // item values are: "All", "Baby Wipes", "Baby Food"
  if (categoryNameHeader) {
    categoryNameHeader.textContent = item;
  }

  if (item === "All") {
    currentProducts = [...AllProducts];
  } else if (item === "Baby Wipes") {
    currentProducts = [...babyWipesProducts];
  } else if (item === "Baby Food") {
    currentProducts = [...babyFoodProducts];
  } else {
    currentProducts = [...AllProducts];
    if (categoryNameHeader) categoryNameHeader.textContent = "All";
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (searchBox) {
    searchBox.addEventListener("input", (e) => {
      ...
      inputText.innerHTML = "";
      if (!inputedItem) return;

      const filteredItems = AllProducts2.filter((p) => p.name.toLowerCase().includes(inputedItem));
  ...
  if (searchBox2) {
    searchBox2.addEventListener("input", (e) => {
      ...
      const filteredItems = AllProducts2.filter((p) => p.name.toLowerCase().includes(inputedItem));
      handleSearchResults(filteredItems, inputedItem, searchedResultsContainer, 50);
    });
  }

  // Search + Menu Toggle functionality
  if (searchIcon) searchIcon.addEventListener("click", () => toggleSearchBar(true));
  if (closeIcon) closeIcon.addEventListener("click", () => toggleSearchBar(false));
  if (menuBtn) menuBtn.addEventListener("click", () => toggleMenu(true));
  if (hiddenClose) hiddenClose.addEventListener("click", () => toggleMenu(false));

  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape") {
      if (searchBar && searchBar.style.display !== "none") toggleSearchBar(false);
      if (hiddenMenu && hiddenMenu.style.display !== "none") toggleMenu(false);
    }
  });
  ...
  if (sidebarClose) {
    sidebarClose.addEventListener("click", () => sidebarOverlay.classList.remove("active"));
  }
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && sidebarOverlay && sidebarOverlay.classList.contains("active")) {
  ...
  if (sortSelect) {
    sortSelect.addEventListener("change", (e) => {
      sortProducts(currentProducts, e.target.value);
    });
  }

  // Product icon clicks
  if (productContainer) productContainer.addEventListener("click", handleProductClick);
